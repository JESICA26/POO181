...
def guardarFrutas():
    if request.method == 'POST':
        ...
        cs.execute('insert into tb_frutas(nombre,temporada,precio,stock) values (%s,%s,%s,%s)', (vnombre,vtemporada,vprecio,vstock))
...
def actualizar(id):
    if request.method == 'POST':
        ...
        curAct.execute('update tb_frutas set nombre= %s, temporada=%s, precio=%s, stock=%s where id=%s',(vnombre,vtemporada,vprecio, vstock,id))
        mysql.connection.commit()
